      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
          ...
          <nav style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <Link href="/coins" className="btn-ghost active" style={{ fontSize: 13, textDecoration: "none", color: "var(--yellow)" }}>Coins</Link>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 18, fontWeight: 800, marginTop: 4, color: "var(--text-primary)" }}>
              $2.48T <span style={{ color: "var(--green)", fontSize: 12, fontWeight: 700 }}>+1.85%</span>
            </div>
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid var(--border)", fontSize: 11, color: "var(--text-secondary)" }}>
                  ...
                  <th style={{ padding: "12px 16px" }}>24H Volume (USD)</th>
                    ...
                    <tr key={coin.symbol} style={{ borderBottom: "1px solid var(--border-light)", transition: "background 0.15s" }} className="pair-row">
                      ...
                      <td style={{ padding: "14px 16px" }}>
                        <span style={{ fontWeight: 600, color: "var(--text-primary)" }}>
                          ${coin.price < 1 ? coin.price.toFixed(6) : coin.price.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                        </span>
                      </td>
                      <td style={{ padding: "14px 16px" }}>
                      ...
                      <td style={{ padding: "14px 16px" }}>
                        <span style={{ fontWeight: 600, color: "var(--text-secondary)" }}>
                          ${coin.volume24h.toLocaleString()}M
                        </span>
                      </td>
                      <td style={{ padding: "14px 16px", textAlign: "right" }}>
                        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                          ...
                          <Link href={`/trade?pair=${coin.symbol}/USDT`} className="btn-yellow bn-tab-sm" style={{ padding: "4px 12px", fontSize: 11, display: "flex", alignItems: "center", gap: 4, textDecoration: "none" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                ...
                <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid var(--border)", borderRadius: 8, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 20, height: 20, borderRadius: "50%", background: "var(--yellow)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: "#000", fontWeight: 800 }}>
              ...
              {activeModalCoin === "USDT" && (
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                    {["TRC20", "ERC20", "Solana"].map(net => (
                      <button key={net} onClick={() => setSelectedNetwork(net)} style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", background: "rgba(0,0,0,0.25)", border: "1px dashed var(--border)", borderRadius: 12, padding: 20, marginTop: 10 }}>
              ...
              <div style={{ background: "rgba(235, 94, 40, 0.08)", border: "1px solid rgba(235, 94, 40, 0.2)", borderRadius: 8, padding: 12, fontSize: 11, color: "var(--yellow)" }}>
                ⚠️ Send only {activeModalCoin} {activeModalCoin === "USDT" && `on ${selectedNetwork}`} network to this address. Sending other assets will result in permanent loss.
              </div>
            </div>
          </div>
        </div>
      ...
      {modalType === "withdraw" && (
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 20 }}>Dispatch digital assets securely from your institutional account.</p>

            {/* Verification status warning */}
            {!kycStatus.includes("Tier-2") ? (
              <div style={{ background: "rgba(239, 68, 68, 0.08)", border: "1px solid rgba(239, 68, 68, 0.25)", borderRadius: 8, padding: 16, color: "var(--red)", fontSize: 12, lineHeight: 1.5, textAlign: "center" }}>
                🚫 <strong>Withdrawal Suspended:</strong> Your account is currently under regulatory basic Tier-1 verification. You must complete <strong>Tier-2 Biometrics Profile Verification</strong> in the KYC & Wallet panel to unlock dispatch limits.
                <Link href="/kyc" onClick={() => setModalType("none")} style={{ display: "block", background: "var(--yellow)", color: "#000", fontWeight: 700, padding: "8px 16px", borderRadius: 6, textDecoration: "none", marginTop: 12 }}>Go to KYC Panel</Link>
              </div>
            ) : (
              <form onSubmit={handleWithdrawalSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div>
                  <label style={{ fontSize: 10, color: "var(--text-secondary)", fontWeight: 700, display: "block", marginBottom: 6 }}>WITHDRAWAL ASSET</label>
                  <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid var(--border)", borderRadius: 8, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 20, height: 20, borderRadius: "50%", background: "var(--yellow)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: "#000", fontWeight: 800 }}>
                      {activeModalCoin.slice(0, 2)}
                    </div>
                    <strong>{activeModalCoin}</strong>
                  </div>
                </div>

                <div>
                  <label style={{ fontSize: 10, color: "var(--text-secondary)", fontWeight: 700, display: "block", marginBottom: 6 }}>RECIPIENT WALLET ADDRESS</label>
                  <input
                    type="text"
                    className="bn-input"
      ...
      {show2faOverlay && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ fontSize: 16, fontWeight: 800, marginBottom: 6 }}>Secure Multi-Factor Authorization</h3>
            <p style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 20 }}>Confirm withdrawal dispatch of {withdrawAmount} {activeModalCoin}.</p>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <button onClick={handleVerifyWithdrawal} className="btn-yellow" style={{ width: "100%", padding: 12, fontWeight: 700, fontSize: 13, marginTop: 10 }}>

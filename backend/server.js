...
app.get("/api/admin/users", async (req, res) => {
  if (pool && isDbConnected) {
    ...
    return res.json({
      success: true,
      users: [
        { id: "usr-101", email: "bunty_trader@exchange.com", kycStatus: "Tier-1 Basic (Email Verified)", kycDocumentUrl: null, kycDocType: null, kycDocNumber: null, kycCountry: null, isMerchant: false, isBlocked: false, blockReason: null, createdAt: (/* @__PURE__ */ new Date()).toISOString() },
        { id: "usr-102", email: "rakesh_bhol@cloudexchange.com", kycStatus: "Tier-2 Verified (Identity Approved)", kycDocumentUrl: "/uploads/dummy_aadhaar.png", kycDocType: "Aadhaar / National ID Card", kycDocNumber: "1234-5678-9012", kycCountry: "India", isMerchant: true, isBlocked: false, blockReason: null, createdAt: (/* @__PURE__ */ new Date()).toISOString() },
        { id: "usr-103", email: "blocked_scammer@spambox.com", kycStatus: "Tier-1 Basic (Email Verified)", kycDocumentUrl: null, kycDocType: null, kycDocNumber: null, kycCountry: null, isMerchant: false, isBlocked: true, blockReason: "Fraudulent dispute transaction", createdAt: (/* @__PURE__ */ new Date()).toISOString() }
      ]
    });
  }
});
...
app.post("/api/admin/balances/adjust", async (req, res) => {
  ...
  if (!userId || !symbol || amount === void 0 || !action) {
  ...
  if (pool && isDbConnected) {
    try {
      ...
      const txid = "adj-" + import_crypto.default.randomBytes(16).toString("hex");
  ...
  if (pool && isDbConnected) {
    try {
      ...
      if (documentImage.startsWith("data:image")) {
        ...
        const savePath = import_path.default.join(__dirname, "../uploads", filename);
...
app.post("/api/p2p/merchant/apply", async (req, res) => {
  ...
  if (pool && isDbConnected) {
    try {
      ...
      const txid = "lock-" + import_crypto.default.randomBytes(16).toString("hex");
      await pool.query(
      ...
      if (result.rows.length > 0) {
        ...
        if (escrow.state !== "RELEASED") {
          ...
          if (buyerRes.rows.length > 0) {
            ...
            const feeAmount = +(parseFloat(escrow.amountUsdt) * 1e-3).toFixed(8);
            const netAmount = +(parseFloat(escrow.amountUsdt) - feeAmount).toFixed(8);
            await pool.query(
